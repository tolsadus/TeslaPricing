...
async function upsertOne(client, row, now) {
...
async function upsert(rows) {
  ...
  let count = 0
  process.stdout.write(`  upserting 0/${rows.length}`)
  for (const row of rows) {
    const client = await pool.connect()
    try {
      await client.query('BEGIN')
      await client.query('SET LOCAL statement_timeout = 0')
      await upsertOne(client, row, now)
      await client.query('COMMIT')
      count++
      process.stdout.write(`\r  upserting ${count}/${rows.length}`)
    } catch (err) {
